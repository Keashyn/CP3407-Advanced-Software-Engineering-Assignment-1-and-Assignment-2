...
import copy
import calendar
import spendee_dashboard
import spendee_expenses
import spendee_budgets
import spendee_categories
import spendee_reports
import spendee_account
...
def index():
    ...
    if request.method == "GET":
        ...
        payers = spendee_account.getPayers(session["user_id"])

        # Get todays date (for quick expense modal)
        date = datetime.today().strftime('%Y-%m-%d')

        # Get the users income
        income = spendee_account.getIncome(session["user_id"])

        # Get current years total expenses for the user
        expenses_year = spendee_dashboard.getTotalSpend_Year(session["user_id"])

        # Get current months total expenses for the user
        expenses_month = spendee_dashboard.getTotalSpend_Month(
            session["user_id"])

        # Get current week total expenses for the user
        expenses_week = spendee_dashboard.getTotalSpend_Week(session["user_id"])

        # Get last 5 expenses for the user
        expenses_last5 = spendee_dashboard.getLastFiveExpenses(
            session["user_id"])

        # Get every budgets spent/remaining for the user
        budgets = spendee_dashboard.getBudgets(session["user_id"])

        # Get weekly spending for the user
        weeks = spendee_dashboard.getLastFourWeekNames()
        spending_week = spendee_dashboard.getWeeklySpending(
            weeks, session["user_id"])

        # Get monthly spending for the user (for the current year)
        spending_month = spendee_dashboard.getMonthlySpending(
            session["user_id"])

        # Get spending trends for the user
        spending_trends = spendee_dashboard.getSpendingTrends(
            session["user_id"])

        # Get payer spending for the user
        payersChart = spendee_reports.generatePayersReport(session["user_id"])
    ...
    # User reached route via POST
        ...
        expenses = spendee_expenses.addExpenses(formData, session["user_id"])
...
def expensehistory():
    ...
    if request.method == "GET":
        ...
        history = spendee_expenses.getHistory(session["user_id"])
    ...
    # User reached route via POST
        ...
        oldExpense = spendee_expenses.getExpense(
        ...
        if userHasSelected_deleteExpense == True:
            ...
            deleted = spendee_expenses.deleteExpense(
            ...
            history = spendee_expenses.getHistory(session["user_id"])
            categories = spendee_categories.getSpendCategories(
                session["user_id"])
            payers = spendee_account.getPayers(session["user_id"])
        ...
        # Update the existing expense record
            ...
            expensed = spendee_expenses.updateExpense(
...
def budgets(year=None):
    ...
    if request.method == "GET":
        ...
        budgets = spendee_budgets.getBudgets(session["user_id"])

        # Get the users total budgeted amount
        budgeted = spendee_budgets.getTotalBudgetedByYear(
    ...
    # User reached route via POST
        ...
        # Delete the budget
        deletedBudgetName = spendee_budgets.deleteBudget(
...
def createbudget():
    ...
    if request.method == "POST":
        ...
        budgetDict = spendee_budgets.generateBudgetFromForm(formData)
        ...
        if "apology" in budgetDict:
            ...
            budget = spendee_budgets.createBudget(
        ...
        # Get the users total budgeted amount
        budgeted = spendee_budgets.getTotalBudgetedByYear(session["user_id"])
...
def updatebudget(urlvar_budgetname):
    ...
    if request.method == "POST":
        ...
        if "apology" in budgetDict:
            ...
            budget = spendee_budgets.updateBudget(
    ...
    # User reached route via GET
        ...
        budgetID = spendee_budgets.getBudgetID(
            urlvar_budgetname, session["user_id"])
        if budgetID is None:
            return apology("'" + urlvar_budgetname + "' budget does not exist")
        else:
            budget = spendee_budgets.getBudgetByID(budgetID, session["user_id"])
        ...
        budget = spendee_budgets.getUpdatableBudget(budget, session["user_id"])
...
def categories():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_newCategory:
            ...
            categoryCount = len(
                spendee_categories.getSpendCategories(session["user_id"]))
            if categoryCount >= 30:
            ...
            categoryID = spendee_categories.getCategoryID(newCategoryName)
            ...
            if categoryID:
                ...
                existingID = spendee_categories.getCategoryID(
                ...
                # Add the category to the users account
                    ...
                    spendee_categories.addCategory_User(
            ...
            # Category does not exist in the DB already - create a new category and then add it to the users account
                ...
                newCategoryID = spendee_categories.addCategory_DB(
                    newCategoryName)
        ...
        if userHasSelected_renameCategory:
            ...
            oldCategoryID = spendee_categories.getCategoryID(oldCategoryName)
            ...
            # Check to see if the *new* name already exists in the database (None == does not exist)
            newCategoryID = spendee_categories.getCategoryID(newCategoryName)
            ...
            if newCategoryID:
                ...
                newCategoryNameFromDB = spendee_categories.getSpendCategoryName(
                    newCategoryID)

                # Rename the category
                spendee_categories.renameCategory(
        ...
        if userHasSelected_deleteCategory:
            ...
            categoryID = spendee_categories.getCategoryID(deleteName)
            ...
            # Delete the category
            spendee_categories.deleteCategory(categoryID, session["user_id"])
    ...
    # User reached route via GET
        ...
        categoryBudgets = spendee_categories.getBudgetsSpendCategories(
            session["user_id"])

        # Generate a single data structure for storing all categories and their associated budgets
        categoriesWithBudgets = spendee_categories.generateSpendCategoriesWithBudgets(
...
def budgetsreport(year=None):
    ...
    budgets = spendee_reports.generateBudgetsReport(session["user_id"], year)
...
def monthlyreport(year=None):
    ...
    monthlySpending = spendee_reports.generateMonthlyReport(
        session["user_id"], year)
...
def spendingreport(year=None):
    ...
    spendingReport = spendee_reports.generateSpendingTrendsReport(
        session["user_id"], year)
...
def payersreport(year=None):
    ...
    payersReport = spendee_reports.generatePayersReport(
        session["user_id"], year)
...
def updateaccount():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_updateIncome:
            ...
            updatedIncome = spendee_account.updateIncome(
        ...
        if userHasSelected_addPayer:
            ...
            # Add the payer
            newPayer = spendee_account.addPayer(newName, session["user_id"])
        ...
        if userHasSelected_renamePayer:
            ...
            # Rename the payer
            renamedPayer = spendee_account.renamePayer(
        ...
        if userHasSelected_deletePayer:
            ...
            # Delete the payer
            deletedPayer = spendee_account.deletePayer(name, session["user_id"])
        ...
        if userHasSelected_updatePassword:
            ...
            updatedPassword = spendee_account.updatePassword(request.form.get(
        ...
        user = spendee_account.getAllUserInfo(session["user_id"])
